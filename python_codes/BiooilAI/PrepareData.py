...
def prepare_data():
    ...
    # Catalyst_Id'ye göre veriyi ayır
    catalyst_data = normalized_data[normalized_data['Catalyst_Id'].notna()]
    non_catalyst_data = normalized_data[normalized_data['Catalyst_Id'].isna()]
    ...
    X_catalyst = catalyst_data[input_cols]
    y_catalyst = catalyst_data[output_cols]
    
    # Non-catalyst verilerini train-test olarak böl (80-20)
    X_non_catalyst = non_catalyst_data[input_cols]
    y_non_catalyst = non_catalyst_data[output_cols]
    
    X_train_non_catalyst, X_test_non_catalyst, y_train_non_catalyst, y_test_non_catalyst = train_test_split(
        X_non_catalyst, y_non_catalyst, test_size=0.2, random_state=42
    )
    
    # Final eğitim ve test setlerini oluştur
    X_train = pd.concat([X_catalyst, X_train_non_catalyst])
    X_test = X_test_non_catalyst
    y_train = pd.concat([y_catalyst, y_train_non_catalyst])
    y_test = y_test_non_catalyst
    ...
    print("\nCatalyst dağılımı:")
    print(f"Eğitim setinde Catalyst_Id olan veri sayısı: {len(X_catalyst)}")
    print(f"Eğitim setinde Catalyst_Id olmayan veri sayısı: {len(X_train_non_catalyst)}")
    print(f"Test setinde Catalyst_Id olmayan veri sayısı: {len(X_test_non_catalyst)}")
    
    return X_train, X_test, y_train, y_test, scaler

def save_processed_data(X_train, X_test, y_train, y_test, file_prefix):
    """İşlenmiş verileri target adına özel CSV dosyaları olarak kaydeder"""
    ...
    print(f"[{file_prefix}] verileri kaydedildi.")
...
def prepare_data():
    ...
    df_norm, input_cols, output_cols, scaler = normalize_features(cleaned_data)
    if not input_cols or not output_cols:
        print("Giriş veya çıkış sütunları eksik, işlem durduruldu.")
        return

    base_prefix = r'C:\@biyokomurlestirme\python_codes\BiooilAI\processed_data'

    for target in output_cols:
        # 1) Null olan satırları çıkar
        df_t = df_norm.dropna(subset=[target]).copy()
        if df_t.empty:
            print(f"{target} için hiç veri yok, atlanıyor.")
            continue

        X = df_t[input_cols]
        y = df_t[[target]]

        # 2) Catalyst'li ve katalizörsüz veriyi ayır
        X_cat = X[df_t['Catalyst_Id'].notna()]
        y_cat = y[df_t['Catalyst_Id'].notna()]

        X_non = X[df_t['Catalyst_Id'].isna()]
        y_non = y[df_t['Catalyst_Id'].isna()]

        # 3) Katalizörsüz veriyi %80-20 böl
        X_train_nc, X_test, y_train_nc, y_test = train_test_split(
            X_non, y_non, test_size=0.2, random_state=42
        )

        # 4) Eğitim seti: katalizörlü + %80'lik kısmı, test: kalan %20
        X_train = pd.concat([X_cat, X_train_nc], ignore_index=True)
        y_train = pd.concat([y_cat, y_train_nc], ignore_index=True)

        # 5) Kaydet
        prefix = f"{base_prefix}_{target}"
        save_processed_data(X_train, X_test, y_train, y_test, prefix)

        # 6) Özet bilgi
        print(f"\n[{target}] Dataset boyutları:")
