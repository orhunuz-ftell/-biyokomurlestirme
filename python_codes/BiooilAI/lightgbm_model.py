...
class BiooilLightGBMPredictor:
    def __init__(self):
        """Model ve sonuçları tutacak değişkenleri başlat"""
        self.lgb_models = {}  # Her çıktı için ayrı model
    ...
    def train_models(self, X_train, X_test, y_train, y_test):
        """Her çıktı değişkeni için ayrı LightGBM modeli eğit"""
        ...
        for output in output_names:
            ...
            try:
                ...
                # Sonuçları kaydet
                self.lgb_models[output] = lgb_model
    ...
    def plot_feature_importance(self, output, top_n=10):
        ...
        plt.title(f'En Önemli {top_n} Özellik - {output} (LightGBM)')
    ...
    def plot_predictions(self, X_test, y_test, output):
        ...
        if output not in self.lgb_models:
            print(f"UYARI: {output} için model bulunamadı!")
            return
        ...
        y_pred = self.lgb_models[output].predict(X_test_clean)
        ...
        plt.title(f'{output} - Tahmin vs Gerçek Değerler (LightGBM)')
        plt.tight_layout()
        plt.show()

    def save_models(self, path='lgb_models'):
        """Eğitilmiş modelleri kaydet"""
        for output, model in self.lgb_models.items():
            joblib.dump(model, f'{path}_{output}.joblib')
        print("LightGBM Modelleri kaydedildi!")
...
if __name__ == "__main__":
    ...
    predictor = BiooilLightGBMPredictor()
    ...
    for output in y_train.columns:
        if output in predictor.lgb_models:  # Check if model exists
            print(f"\n{output} için özellik önem dereceleri (LightGBM):")
